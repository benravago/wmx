...
#if CONFIG_USE_COMPOSITE
#include <X11/extensions/Xcomposite.h>
#endif
#if CONFIG_CLEAR_NUMLOCK
#include <X11/XKBlib.h>
void clearNumLock(Display*);
#endif
...
implementPList(ClientList, Client); // @suppress("Method cannot be resolved")
implementList(AtomList, Atom);  // @suppress("Method cannot be resolved")
    ...
    m_altModMask(0) // later
{
    ...
    if (!setlocale(LC_ALL, "")) {
        fprintf(stderr, "Warning: locale not supported by C library, locale unchanged\n");
    }
    if (!XSupportsLocale()) {
        fprintf(stderr, "Warning: locale not supported by Xlib, locale set to C\n");
        setlocale(LC_ALL, "C");
    }
    if (!XSetLocaleModifiers("")) {
        fprintf(stderr, "Warning: X locale modifiers not supported, using default\n");
    }
    ...
    if (CONFIG_USE_KEYBOARD) {
        fprintf(stderr, "Keyboard controls available.\n");
    } else {
        fprintf(stderr, "No keyboard controls.\n");
    }
    ...
    fprintf(stderr, "Operating system locale is \"%s\".\n", ret_setlocale ? ret_setlocale : "(NULL)");
    fprintf(stderr, "NETWM compliant.\n");
    fprintf(stderr, "Command menu taken from ");
    if (wmxdir == NULL) {
    ...
    //!!! what is this??
    Atoms::netwm_winDesktopButtonProxy = XInternAtom(m_display, "_WIN_DESKTOP_BUTTON_PROXY", False);
    //!!! "replaced with _NET_WM_WINDOW_TYPE functional hint":
    ...
    initialiseScreen();
    if (m_screensTotal > 1) {
        fprintf(stderr, "Detected %d screens.\n", m_screensTotal);
    }

#if CONFIG_USE_COMPOSITE
    int ev, er;
    if (XCompositeQueryExtension(m_display, &ev, &er)) {
        fprintf(stderr, "Enabling composite extension.\n");
...
unsigned long WindowManager::allocateColour(int screen, const char *name, const char *desc) {
    XColor nearest, ideal;
    if (!XAllocNamedColor(display(), DefaultColormap(display(), screen), name, &nearest, &ideal)) {
        char error[100];
...
Time WindowManager::timestamp(Boolean reset) {
    ...
    if (m_currentTime == CurrentTime) {
        XEvent event;
        XChangeProperty(m_display, root(), Atoms::wmx_running, Atoms::wmx_running, 8, PropModeAppend, (unsigned char*) "", 0);
...
Client* WindowManager::windowToClient(Window w, Boolean create) {
    ...
    if (!create) {
        ...
        int x_clip = 0;
        int y_clip = 0;
        (void) XShapeQueryExtents(m_display, w, &bounding_shape, &x_bounding, &y_bounding, &w_bounding, &h_bounding, &clip_shape, &x_clip, &y_clip, &w_clip, &h_clip);
...
void WindowManager::spawn(char *name, char *file) {
    ...
    if (fork() == 0) {
        if (fork() == 0) {
            ...
            // if you don't have putenv, miss out this next conditional and its contents
            if (displayName && (displayName[0] != '\0')) {
                char *c;
                char *pstring = (char*) malloc(strlen(displayName) + 11 + numdigits(screen()));
            ...
            if (CONFIG_EXEC_USING_SHELL) {
                if (file) {
                    execl(m_shell, m_shell, "-c", file, NULL);
                } else {
                    execl(m_shell, m_shell, "-c", name, NULL);
                }
...
void WindowManager::netwmInitialiseCompliance() {
    // NOTE that this has been altered to coexist with the multihead code;
	// but we're only using screen 0 here as I have no idea how NETWM copes with multiheads --cc

    // most of this is taken verbatim from
    // http://www.gnome.org/devel/gnomewm

    // this part is to tell NETWM we are compliant.
	// the window is needed to tell NETWM that wmx has exited (i think).
    m_netwmCheckWin = XCreateSimpleWindow(m_display, m_root[0], -200, -200, 5, 5, 0, 0, 0);

    XChangeProperty(m_display, m_root[0], Atoms::netwm_supportingWmCheck, XA_WINDOW, 32, PropModeReplace, (unsigned char*) &m_netwmCheckWin, 1);
    XChangeProperty(m_display, m_netwmCheckWin, Atoms::netwm_supportingWmCheck, XA_WINDOW, 32, PropModeReplace, (unsigned char*) &m_netwmCheckWin, 1);
    XChangeProperty(m_display, m_root[0], Atoms::netwm_wmName, XA_STRING, 8, PropModeReplace, (unsigned char*) "wmx", 1);
    XChangeProperty(m_display, m_netwmCheckWin, Atoms::netwm_wmName, XA_STRING, 8, PropModeReplace, (unsigned char*) "wmx", 1);

    // Also going to add the desktop button proxy stuff to the same window.
    // This is an old property, but it's not clear how it has been superseded
    // -- see e.g. http://developer.gnome.org/doc/standards/wm/c71.html

    XChangeProperty(m_display, m_root[0], Atoms::netwm_winDesktopButtonProxy, XA_CARDINAL, 32, PropModeReplace, (unsigned char*) &m_netwmCheckWin, 1);
    XChangeProperty(m_display, m_netwmCheckWin, Atoms::netwm_winDesktopButtonProxy, XA_CARDINAL, 32, PropModeReplace, (unsigned char*) &m_netwmCheckWin, 1);
    ...
    XChangeProperty(m_display, m_root[0], Atoms::netwm_supported, XA_ATOM, 32,
    PropModeReplace, (unsigned char*) supported.array(0, supported.count()), supported.count());
}
...
void WindowManager::netwmUpdateWindowList() {
    ...
    XChangeProperty(m_display, m_root[0], Atoms::netwm_clientList, XA_WINDOW, 32, PropModeReplace, (unsigned char*) byAge, count);
    delete[] byAge;
    netwmUpdateStackingOrder();
    netwmUpdateActiveClient();
}
...
void WindowManager::netwmUpdateStackingOrder() {
    ...
    for (int layer = 0; layer < MAX_LAYER; ++layer) {
        ...
        for (int i = top - 1; i >= 0; --i) {
            Client *c = m_orderedClients[layer].item(i);
            if (c->isWithdrawn() || c->isKilled() || c->isHidden()) {
                continue;
            }
            byStacking[count++] = c->window();
    ...
    XChangeProperty(m_display, m_root[0], Atoms::netwm_clientListStacking, XA_WINDOW, 32, PropModeReplace, (unsigned char*) byStacking, count);
    delete[] byStacking;
}
...
void WindowManager::netwmUpdateActiveClient() {
    ...
    XChangeProperty(m_display, m_root[0], Atoms::netwm_activeWindow, XA_WINDOW, 32, PropModeReplace, (unsigned char*) &val, 1);
}

void WindowManager::printClientList() {
    ...
    for (int i = 0; i < m_clients.count(); ++i) {
        ...
        printf("wmx: Client %d (address %p)%s\n", i + 1, m_clients.item(i), inHidden ? " [hidden]" : "");
...
#if CONFIG_CLEAR_NUMLOCK
void clearNumLock(Display *dpy) {
    unsigned int mask;
    XkbDescPtr kb = XkbGetKeyboard(dpy, XkbAllComponentsMask, XkbUseCoreKbd);
    if (kb) {
        for (int i = 0; i < XkbNumVirtualMods; i++) {
            char *vmod = XGetAtomName(kb->dpy, kb->names->vmods[i]);
            if (vmod && strcmp("NumLock", vmod) == 0) {
                XkbVirtualModsToReal(kb, 1 << i, &mask);
                break;
            }
        }
    ...
    if (mask) {
        XkbLockModifiers(dpy, XkbUseCoreKbd, mask, mask); // on
        XkbLockModifiers(dpy, XkbUseCoreKbd, mask, 0); // off
    }
}
#endif
