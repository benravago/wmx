...
public:
    ...
    void lower();
    void raiseOrLower(); // without activating, raise if it isn't top; otherwise lower.
    ...
    // These are the only accepted calls for feedback:
    // the Manager should *not* call directly into the Border
    void showFeedback();
    ...
    Boolean isTransient() {
        return ((m_transient != None) || (m_shaped));
    }
    Boolean isSticky() {
        return m_sticky;
    }
    Boolean skipsFocus() {
        return m_skipFocus || isNonFocusable();
    } // !!!merge
    Boolean isFocusOnClick() {
        return (m_focusOnClick || (layer() < FIRST_FOCUS_FOLLOWS_LAYER));
    }
    Boolean isMovable() {
        return m_movable;
    }
    ...
    Boolean isBorderless() {
        return ((layer() < FIRST_DECORATED_LAYER) || (layer() > LAST_DECORATED_LAYER));
    }

    // Client should not receive focus (panel, desktop etc.)
    Boolean isNonFocusable() {
        return ((layer() > LAST_FOCUSABLE_LAYER));
    }
    ...
    Boolean isFullHeight() {
        return m_isFullHeight;
    }
    void makeThisNormalHeight() {
        m_isFullHeight = False;
    }
    void makeThisNormalWidth() {
        m_isFullWidth = False;
    }
...
private:
    ...
    Window m_wroot;
    int m_screen;
    Boolean m_doSomething; // Become true if move() or resize() made effect to this client.
